#!/usr/bin/env python3
"""ACI tenant / VRF / ESG diagrams → PowerPoint (v10).

Same sources and layout as v9. Default output is a .pptx slide that
fills a widescreen (16:9) canvas. PNG/SVG/HTML/JSON remain available.
...
1. Retrieve tenant.json from APIC
   python3 diagrams/render_v10_tenant_diagram.py --no-prompt --tenant common --from-apic

2. Path to a downloaded tenant.json
   python3 diagrams/render_v10_tenant_diagram.py --no-prompt --tenant common --tenant-json diagrams/tn-common/tenant.json

3. Retrieve tenant.json from APIC + path to NAC configuration
   python3 diagrams/render_v10_tenant_diagram.py --no-prompt --tenant common --from-apic --nac path/to/configuration.nac.yaml

4. Path to a downloaded tenant.json + path to NAC configuration
   python3 diagrams/render_v10_tenant_diagram.py --no-prompt --tenant common --tenant-json diagrams/tn-common/tenant.json --nac path/to/configuration.nac.yaml
...
Layout
======
Every VRF in the tenant gets its own orange column (L3Outs, BDs, APs / ESGs /
EPGs, vzAny). Mixed-VRF application profiles are split so each column only
shows the objects that belong to that VRF. Two-VRF tenants keep the historical
outside / inside split; three or more VRFs share the canvas equally (the
canvas grows if needed). Every y coordinate is measured before anything is
drawn (see _layout()).

Output formats
==============
--format pptx | png | svg | both | html | json | all
Default is pptx: one widescreen (16:9) slide with the diagram fitted.
Requires python-pptx (python3 -m pip install python-pptx).
"""
...
LEGEND_X = 1180
LANE_MIN_W = 1080         # minimum VRF column width when 3+ VRFs share the canvas
LANE_GAP = 16
...
@dataclass
class VrfLane:
    """One VRF column: L3Outs, BDs, APs, and optional vzAny."""
    name: str
    role: str = ""
    l3outs: list[str] = field(default_factory=list)
    bds: list[BD] = field(default_factory=list)
    aps: list[AP] = field(default_factory=list)
    vz_consumers: list[str] = field(default_factory=list)
...
class Model:
    ...
    lanes: list[VrfLane] = field(default_factory=list)


def order_vrfs(names: list[str]) -> list[str]:
    """Stable column order: outside (vrf-02) first, then vrf-01, then the rest."""
    seen = [n for n in dict.fromkeys(names) if n]
    if not seen:
        return ["vrf-01"]
    ordered: list[str] = []
    for preferred in ("vrf-02", "vrf-01"):
...
def choose_vrfs(names: list[str]) -> tuple[str, str | None]:
    """Pick the inside (primary) and outside (secondary) VRF.

    Extra VRFs (vrf-03, …) are not dropped — they are drawn as their own lanes.
    """
    ordered = order_vrfs(names)
    primary = "vrf-01" if "vrf-01" in ordered else ordered[0]
    secondary = "vrf-02" if "vrf-02" in ordered and "vrf-02" != primary else None
    if secondary is None and len(ordered) > 1:
        secondary = next((n for n in ordered if n != primary), None)
    return primary, secondary


def vrf_role(name: str, primary: str, secondary: str | None) -> str:
    if secondary and name == secondary:
        return "outside"
    if name == primary:
        return "inside"
    return ""
...
def build_lanes(
    ...
    l3_by_vrf: dict[str, list[str]],
    bds_by_vrf: dict[str, list[BD]],
    vz_by_vrf: dict[str, list[str]],
) -> list[VrfLane]:
    lanes = []
    for name in order:
        aps = list(aps_by_vrf.get(name) or [])
        if name == primary:
            _out, center, right = layout_aps({primary: aps}, primary, None)
            aps = center + right
        lanes.append(
            VrfLane(
                name=name,
                role=vrf_role(name, primary, secondary),
                l3outs=list(l3_by_vrf.get(name) or []),
                bds=list(bds_by_vrf.get(name) or []),
                aps=aps,
                vz_consumers=list(vz_by_vrf.get(name) or []),
            )
        )
    return lanes


def canvas_width_for(n_lanes: int) -> int:
    n = max(1, n_lanes)
    if n <= 2:
        return W
    return max(W, 64 + n * LANE_MIN_W + (n - 1) * LANE_GAP)


def model_lanes(model: Model) -> list[VrfLane]:
    """Prefer parsed lanes; fall back to the historical two-bucket Model fields."""
    if model.lanes:
        return model.lanes
    primary = model.vrf_primary or "vrf-01"
    secondary = model.vrf_secondary or ""
    lanes = []
    if secondary:
        lanes.append(
            VrfLane(
                name=secondary,
                role="outside",
                l3outs=_l3_labels(model, "02"),
                aps=list(model.aps_vrf02),
            )
        )
    lanes.append(
        VrfLane(
            name=primary,
            role="inside" if secondary else "",
            l3outs=_l3_labels(model, "01"),
            bds=list(model.bds),
            aps=list(model.aps_vrf01_center) + list(model.aps_vrf01_right),
            vz_consumers=list(model.vz_consumers),
        )
    )
    return lanes


def iter_aps(model: Model):
    """Yield (vrf_name, AP) for every application profile in the model."""
    if model.lanes:
    ...
    for ap in model.aps_vrf02:
        yield model.vrf_secondary or "", ap
    for ap in model.aps_vrf01_center + model.aps_vrf01_right:
        yield model.vrf_primary, ap
...
def _nac_esg(e: dict) -> ESG:
    ...
    return ESG(e["name"], sels, cons, provs)


def _nac_bd(bd: dict) -> BD:
...
def parse_yaml(path: Path, tenant: str) -> Model:
    ...
    # Collect every VRF this tenant declares or references.
    ...
    for ap in tn.get("application_profiles") or []:
        for e in ap.get("endpoint_security_groups") or []:
            if e.get("vrf"):
                vrf_names.append(e["vrf"])
    primary, secondary = choose_vrfs(vrf_names)
    order = order_vrfs(vrf_names)
    ...
    l3_by_vrf = {name: l3_labels(name) for name in order}
    l3_01 = l3_by_vrf.get(primary) or []
    l3_02 = l3_by_vrf.get(secondary) or [] if secondary else []

    vz_by_vrf: dict[str, list[str]] = {}
    attachments: list[tuple[str, str, str]] = []
    for vrf in tn.get("vrfs") or []:
        name = vrf.get("name")
        if not name:
            continue
        cons = list((vrf.get("contracts") or {}).get("consumers") or [])
        vz_by_vrf[name] = cons
        for c in cons:
            attachments.append((f"vzAny ({name})", "C", c))
    vz = list(vz_by_vrf.get(primary) or [])

    bds_by_vrf: dict[str, list[BD]] = defaultdict(list)
    bd_vrf = {}
    for bd in tn.get("bridge_domains") or []:
        if bd.get("name") == "6.6.6.0_24":
            continue
        vrf = bd.get("vrf") or primary
        bd_vrf[bd["name"]] = vrf
        bds_by_vrf[vrf].append(_nac_bd(bd))
    ...
    for ap in tn.get("application_profiles") or []:
        ...
        for i, epg in enumerate(ap.get("endpoint_groups") or []):
            vrf = bd_vrf.get(epg.get("bridge_domain")) or primary
            buckets.setdefault(vrf, AP(ap["name"])).epgs.append(
                (epg["name"].replace("_", "/"), epg_colors[i % len(epg_colors)])
            )
        ...
        for e in ap.get("endpoint_security_groups") or []:
            vrf = e.get("vrf") or primary
            esg = _nac_esg(e)
            buckets.setdefault(vrf, AP(ap["name"])).esgs.append(esg)
            for c in esg.consumers:
                attachments.append((f"ESG {esg.name}", "C", c))
            for p in esg.providers:
                attachments.append((f"ESG {esg.name}", "P", p))
        for vrf, ap_obj in buckets.items():
            aps_by_vrf[vrf].append(ap_obj)

    lanes = build_lanes(
        order, primary, secondary, aps_by_vrf, l3_by_vrf, bds_by_vrf, vz_by_vrf
    )
    aps_v2, center, right = layout_aps(aps_by_vrf, primary, secondary)
    bds = [bd for ln in lanes for bd in ln.bds]
...
def fetch_apic(tenant: str, host: str, user: str, password: str) -> dict:
    ...
    subtree = get(
        ...
        "fvEPgSelector,fvRsMatchEPg,l3extRsEctx,fvRsScope,fvRsCtx,fvRsBd,"
...
def parse_apic(subtree: dict, tenant: str, apic_host: str) -> Model:
    ...
    vrf_names = [a.get("name", "") for a in by.get("fvCtx", [])]
    primary, secondary = choose_vrfs(vrf_names)
    ...
    for r in by.get("l3extRsEctx", []):
        ...
        if l3_vrf[out]:
            vrf_names.append(l3_vrf[out])
    l3_by_vrf: dict[str, list[str]] = defaultdict(list)
    for n, v in l3_vrf.items():
        l3_by_vrf[v or primary].append(f"{l3_alias.get(n, n)}    {n}")
    if not any(l3_by_vrf.values()) and l3_alias:
        for n in l3_alias:
            if primary in n:
                l3_by_vrf[primary].append(f"{l3_alias.get(n, n)}    {n}")
            elif secondary and secondary in n:
                l3_by_vrf[secondary].append(f"{l3_alias.get(n, n)}    {n}")
        if not any(l3_by_vrf.values()) and not secondary:
            for n in sorted(l3_alias):
                l3_by_vrf[primary].append(f"{l3_alias.get(n, n)}    {n}")
    l3_01 = list(l3_by_vrf.get(primary) or [])
    l3_02 = list(l3_by_vrf.get(secondary) or []) if secondary else []

    vz_by_vrf: dict[str, list[str]] = defaultdict(list)
    for r in by.get("vzRsAnyToCons", []):
        dn = r.get("dn") or ""
        if "/ctx-" not in dn:
            continue
        vrf = dn.split("/ctx-")[1].split("/")[0]
        vz_by_vrf[vrf].append(r["tnVzBrCPName"])
        vrf_names.append(vrf)
    for vrf in list(vz_by_vrf):
        vz_by_vrf[vrf] = sorted(set(vz_by_vrf[vrf]))
    vz = list(vz_by_vrf.get(primary) or [])

    bd_vrf_map = {}
    for r in by.get("fvRsCtx", []):
        dn = r.get("dn") or ""
        if "/BD-" not in dn:
            continue
        bd = dn.split("/BD-")[1].split("/")[0]
        vrf = r.get("tnFvCtxName") or (r.get("tDn") or "").rsplit("/ctx-", 1)[-1]
        bd_vrf_map[bd] = vrf
        if vrf:
            vrf_names.append(vrf)

    epg_bd = {}
    for r in by.get("fvRsBd", []):
        dn = r.get("dn") or ""
        if "/epg-" not in dn:
            continue
        epg = dn.split("/epg-")[1].split("/")[0]
        epg_bd[epg] = r.get("tnFvBDName") or (r.get("tDn") or "").rsplit("/BD-", 1)[-1]
    ...
    for b in by.get("fvBD", []):
        ...
        vrf = bd_vrf_map.get(b["name"]) or primary
        obj = BD(b["name"].replace("_", "/"), f"GW  {ip}   {flags}".rstrip(), vrf=vrf)
        bds_by_vrf[vrf].append(obj)
        bds.append(obj)

    def _dn_ap(dn: str) -> str:
        return dn.split("/ap-")[1].split("/")[0] if "/ap-" in (dn or "") else ""

    def _dn_esg(dn: str) -> str:
        return dn.split("/esg-")[1].split("/")[0] if "/esg-" in (dn or "") else ""

    def _dn_epg(dn: str) -> str:
        return dn.split("/epg-")[1].split("/")[0] if "/epg-" in (dn or "") else ""

    # Key ESG data by (AP, ESG). The same ESG name can exist in more than one AP.
    selectors = defaultdict(list)
    for s in by.get("fvExternalSubnetSelector", []):
        ap, esg = _dn_ap(s["dn"]), _dn_esg(s["dn"])
        if not esg:
            continue
        shared = "   shared" if str(s.get("shared") or "").lower() in ("yes", "true") else ""
        selectors[(ap, esg)].append(f"IP selector  {s.get('ip')}{shared}")
    for s in by.get("fvRsMatchEPg", []):
        esg_ap, esg = _dn_ap(s["dn"]), _dn_esg(s["dn"])
        if not esg:
            continue
        tdn = s.get("tDn") or ""
        epg = tdn.rsplit("/epg-", 1)[-1] if "/epg-" in tdn else ""
        epg_ap = _dn_ap(tdn)
        selectors[(esg_ap, esg)].append(f"EPG selector  {epg_ap} / {epg}")
    ...
    for r in by.get("fvRsCons", []):
        ...
        dn = r.get("dn") or ""
        if "/esg-" in dn:
            esg_cons[(_dn_ap(dn), _dn_esg(dn))].append(name)
        elif "/epg-" in dn:
            epg_cons[(_dn_ap(dn), _dn_epg(dn))].append(name)
    for r in by.get("fvRsProv", []):
        name = r.get("tnVzBrCPName")
        dn = r.get("dn") or ""
        if "/esg-" in dn:
            esg_provs[(_dn_ap(dn), _dn_esg(dn))].append(name)
        elif "/epg-" in dn:
            epg_provs[(_dn_ap(dn), _dn_epg(dn))].append(name)
    ...
    for r in by.get("fvRsScope", []):
        ...
        if not esg:
            continue
        esg_vrf[(_dn_ap(dn), esg)] = r.get("tnFvCtxName") or (r.get("tDn") or "").rsplit("/ctx-", 1)[-1]
    ...
    attachments = []
    for vrf, cons in vz_by_vrf.items():
        for c in cons:
            attachments.append((f"vzAny ({vrf})", "C", c))
    ap_esgs_vrf: dict[tuple[str, str], list[ESG]] = defaultdict(list)
    for e in by.get("fvESg", []):
        ap = e["dn"].split("/ap-")[1].split("/")[0]
        name = e["name"]
        vrf = esg_vrf.get((ap, name)) or default_vrf_for_ap(ap)
        if vrf:
            vrf_names.append(vrf)
        key = (ap, name)
        esg = ESG(name, selectors.get(key, []), esg_cons.get(key, []), esg_provs.get(key, []))
        ap_esgs_vrf[(ap, vrf)].append(esg)
        for c in esg.consumers:
            attachments.append((f"ESG {name}", "C", c))
        for p in esg.providers:
            attachments.append((f"ESG {name}", "P", p))

    ap_epgs_vrf: dict[tuple[str, str], list[tuple[str, tuple]]] = defaultdict(list)
    epg_colors = [RED, L3, BD_BLUE, ESG_GREEN]
    ...
    for a in by.get("fvAEPg", []):
        ...
        epg_i[ap] += 1
        vrf = bd_vrf_map.get(epg_bd.get(a["name"], ""), "") or primary
        if vrf:
            vrf_names.append(vrf)
        ap_epgs_vrf[(ap, vrf)].append((a["name"].replace("_", "/"), color))
        for c in epg_cons.get((ap, a["name"]), []):
            attachments.append((f"EPG {a['name']}", "C", c))
        for p in epg_provs.get((ap, a["name"]), []):
            attachments.append((f"EPG {a['name']}", "P", p))

    aps_by_vrf: dict[str, list[AP]] = defaultdict(list)
    ap_names = sorted({k[0] for k in set(ap_esgs_vrf) | set(ap_epgs_vrf)})
    for ap_name in ap_names:
        vrfs_here = sorted({k[1] for k in set(ap_esgs_vrf) | set(ap_epgs_vrf) if k[0] == ap_name})
        for vrf in vrfs_here:
            aps_by_vrf[vrf].append(
                AP(
                    ap_name,
                    ap_esgs_vrf.get((ap_name, vrf), []),
                    ap_epgs_vrf.get((ap_name, vrf), []),
                )
            )
    order = order_vrfs(vrf_names)
    for name in order:
        l3_by_vrf.setdefault(name, [])
    lanes = build_lanes(
        order, primary, secondary, aps_by_vrf, l3_by_vrf, bds_by_vrf, vz_by_vrf
    )
    aps_v2, center, right = layout_aps(aps_by_vrf, primary, secondary)
...
def _layout_table(model: Model, canvas_w: int = W) -> tuple[list, int]:
    ...
    prov_w = (canvas_w - 40) - TBL_X["prov"] - 20
    out = []
...
def _layout(model: Model) -> dict:
    ...
    One orange column per VRF. Two-VRF tenants keep the historical
    outside / inside split; three or more share the canvas equally.
    """
    lanes = model_lanes(model)
    n = max(1, len(lanes))
    canvas_w = canvas_width_for(n)
    frame_r = canvas_w - 32

    if n == 1:
        frames_xy = [(FRAME_L, frame_r)]
    elif n == 2:
        frames_xy = [(FRAME_L, DUAL_SPLIT), (920, frame_r)]
    else:
        inner = frame_r - FRAME_L
        col_w = int((inner - LANE_GAP * (n - 1)) / n)
        frames_xy = []
        x = FRAME_L
        for _ in range(n):
            frames_xy.append((x, x + col_w))
            x += col_w + LANE_GAP

    frames = []
    max_bottom = 640
    for lane, (x0, x1) in zip(lanes, frames_xy):
        content_x = x0 + PAD
        content_w = max(80, (x1 - x0) - 2 * PAD)
        l3_w = min(620 if n >= 2 else 1600, content_w)
        l3_h = 44 * max(len(lane.l3outs), 1)
        vz_h = vzany_height(lane.vz_consumers) if lane.vz_consumers else 0
        vz_w = min(VZ_W, content_w) if vz_h else 0
        can_side = bool(vz_h) and (l3_w + 24 + vz_w) <= content_w

        show_k8s = (
            n == 2
            and lane.role == "inside"
            and any("k8s" in (lab or "").lower() for lab in lane.l3outs)
        )
        k8s_x, k8s_w, k8s_top = content_x + l3_w + 70, 500, BAND_TOP - 42
        vz_x = x1 - PAD - vz_w if vz_h else 0
        if show_k8s and vz_h:
            show_k8s = (k8s_x + k8s_w) <= (vz_x - 20)
        if not show_k8s:
            k8s_x = k8s_w = k8s_top = 0

        if can_side:
            header_bottom = max(
                BAND_TOP + l3_h,
                (VZ_TOP + vz_h) if vz_h else 0,
                (k8s_top + 156) if show_k8s else 0,
            )
            vz_y = VZ_TOP
        else:
            header_bottom = BAND_TOP + l3_h
            if show_k8s:
                header_bottom = max(header_bottom, k8s_top + 156)
            vz_x = content_x
            vz_y = header_bottom + PAD if vz_h else 0
            if vz_h:
                header_bottom = vz_y + vz_h

        bd_top = header_bottom + PAD
        bd_w = min(BD_W, content_w)
        per_row = max(1, int((content_w + BD_GAP) // (bd_w + BD_GAP)))
        bd_rows = -(-len(lane.bds) // per_row) if lane.bds else 0
        aps_top = bd_top + ((56 * (bd_rows - 1) + 46 + PAD) if bd_rows else 0)

        ap_min = min(AP_MIN_W, content_w)
        n_cols = 1
        for nc in (3, 2, 1):
            if (content_w - AP_GAP * (nc - 1)) / nc >= ap_min:
                n_cols = nc
                break
        ap_col_w = int((content_w - AP_GAP * (n_cols - 1)) / n_cols)
        assign, tallest = pack_columns([ap_height(a) for a in lane.aps], n_cols, AP_GAP)
        bottom = aps_top + tallest + PAD
    ...
    # Align BD / AP rows across VRF columns so a tall header (many L3Outs +
    # vzAny) does not leave the other columns starting higher up.
    if frames:
        shared_bd = max(fr["bd_top"] for fr in frames)
        max_bottom = 640
        for fr in frames:
            delta = shared_bd - fr["bd_top"]
            if delta:
                fr["bd_top"] += delta
                fr["aps_top"] += delta
                fr["bottom"] += delta
            max_bottom = max(max_bottom, fr["bottom"])

    table_rows, table_h = _layout_table(model, canvas_w)
    return {
        "canvas_w": canvas_w,
        "frames": frames,
        "topo_bottom": max_bottom,
...
def _draw_contract_table(draw, model: Model, plan: dict, top: int, canvas_h: int) -> None:
    canvas_w = plan.get("canvas_w", W)
    draw.rounded_rectangle((FRAME_L, top, canvas_w - 32, canvas_h - 52), radius=6,
    ...
    hy = top + 50
    draw.rectangle((40, hy, canvas_w - 40, hy + 26), fill=(236, 245, 228))
    ...
    for i, (contract, scope, cl, pl, h) in enumerate(rows):
        if i % 2 == 0:
            draw.rectangle((40, ry - 1, canvas_w - 40, ry + h - 1), fill=(250, 250, 250))
    ...
    if not rows:
        ...
        ry += 28
    dtext(draw, (48, ry + 8), model.notes, F(12), MUTED, canvas_w - 120)
...
def _draw_vrf_lane(draw, fr: dict, topo_bottom: int) -> None:
    """One orange VRF column: L3Outs, optional vzAny, BDs, then APs."""
    lane = fr["lane"]
    draw.rounded_rectangle(
        (fr["x0"], 74, fr["x1"], topo_bottom),
        radius=6,
        fill=None,
        outline=VRF_ORANGE,
        width=2,
    )
    router(draw, fr["content_x"], 90)
    role = f"  ({lane.role})" if lane.role else ""
    dtext(
        draw,
        (fr["content_x"] + 46, 92),
        f"{lane.name}{role}",
    ...
    y = BAND_TOP
    for label in lane.l3outs:
        y += l3out_box(draw, fr["content_x"], y, fr["l3_w"], label) + 10
    if not lane.l3outs:
        draw.text(
            (fr["content_x"], BAND_TOP + 8),
            "no L3Out attached to this VRF",
            font=F(13),
            fill=MUTED,
        )
    ...
    if fr.get("show_k8s"):
        draw.line(
            (fr["content_x"] + fr["l3_w"] + 8, BAND_TOP + 17, fr["k8s_x"] - 6, BAND_TOP + 17),
            fill=L3,
            width=3,
        )
        k8s_stack(draw, fr["k8s_x"], fr["k8s_top"], fr["k8s_w"])

    if lane.vz_consumers:
        vzany_box(draw, fr["vz_x"], fr["vz_y"], fr["vz_w"], lane.vz_consumers, lane.name)

    bx, by = fr["content_x"], fr["bd_top"]
    for i, bd in enumerate(lane.bds):
        if i and i % fr["per_row"] == 0:
            bx = fr["content_x"]
            by += 56
        bd_box(draw, bx, by, fr["bd_w"], bd)
        bx += fr["bd_w"] + BD_GAP

    col_y = [fr["aps_top"]] * fr["n_cols"]
    for ap, col in zip(fr["aps"], fr["assign"]):
        x = fr["content_x"] + col * (fr["col_w"] + AP_GAP)
        col_y[col] += draw_ap(draw, x, col_y[col], fr["col_w"], ap) + AP_GAP
...
def render(model: Model, outfile: Path, fmt: str = "png"):
    ...
    img, d = _new_canvas(canvas_w, canvas_h, fmt)
    ...
    # tenant frame
    d.rounded_rectangle((16, 58, canvas_w - 16, canvas_h - 44), radius=8,
                        fill=None, outline=TENANT_GREEN, width=3)

    for fr in plan["frames"]:
        _draw_vrf_lane(d, fr, topo_bottom)
    ...
    d.text((canvas_w - 110, canvas_h - 36), "cisco", font=F(18, bold=True), fill=NAVY)
...
def write_pptx(model: Model, outfile: Path) -> Path:
    """One widescreen slide with the rendered diagram fitted to the page."""
    try:
        from pptx import Presentation
        from pptx.util import Inches
    except ImportError as exc:
        raise SystemExit(
            "PowerPoint output needs python-pptx. Install with:\n"
            "  python3 -m pip install python-pptx"
        ) from exc

    if outfile.suffix.lower() != ".pptx":
        outfile = outfile.parent / (outfile.name + ".pptx")
    outfile.parent.mkdir(parents=True, exist_ok=True)

    png_tmp = outfile.with_name(outfile.stem + ".__slide.png")
    try:
        render(model, png_tmp, "png")
        with Image.open(png_tmp) as im:
            iw, ih = im.size

        slide_w, slide_h, margin = 13.333333, 7.5, 0.18
        avail_w, avail_h = slide_w - 2 * margin, slide_h - 2 * margin
        if iw / ih > avail_w / avail_h:
            pic_w, pic_h = avail_w, avail_w * ih / iw
        else:
            pic_h, pic_w = avail_h, avail_h * iw / ih
        ...
        slide.shapes.add_picture(
            str(png_tmp), Inches(left), Inches(top), Inches(pic_w), Inches(pic_h)
        )
        notes = slide.notes_slide.notes_text_frame
        notes.text = f"{model.title}\n{model.subtitle}\n{model.notes}"
        prs.save(str(outfile))
        print(f"wrote {outfile}  {slide_w:.3f}x{slide_h:.1f} in  ({iw}x{ih} diagram)")
    finally:
        if png_tmp.exists():
            png_tmp.unlink()
    return outfile
...
def parse_snapshot_data(snap: dict, fallback_tenant: str) -> Model:
    ...
    vrf_names += [b.get("vrf") for b in snap.get("bds", []) if b.get("vrf")]
    vrf_names += [e.get("vrf") for e in snap.get("epgs", []) if e.get("vrf")]
    primary, secondary = choose_vrfs(vrf_names or ["vrf-01", "vrf-02"])
    order = order_vrfs(vrf_names or [primary])

    vz_by_vrf: dict[str, list[str]] = defaultdict(list)
    if snap.get("vz_consumers_vrf01"):
        vz_by_vrf[primary] = list(snap["vz_consumers_vrf01"])
    for k, v in (snap.get("vz_consumers") or {}).items():
        vz_by_vrf[k] = list(v)
    ...
    aps_by_vrf: dict[str, list[AP]] = defaultdict(list)
    ap_buckets: dict[tuple[str, str], AP] = {}

    def ap_for(ap_name: str, vrf: str) -> AP:
        key = (ap_name, vrf)
        if key not in ap_buckets:
            ap_buckets[key] = AP(ap_name)
            aps_by_vrf[vrf].append(ap_buckets[key])
        return ap_buckets[key]

    for e in snap.get("esgs", []):
        vrf = e.get("vrf") or primary
        esg = ESG(e["name"], e.get("selectors", []), e.get("consumers", []), e.get("providers", []))
        ap_for(e["ap"], vrf).esgs.append(esg)
        for c in esg.consumers:
            attachments.append((f"ESG {esg.name}", "C", c))
        for p in esg.providers:
            attachments.append((f"ESG {esg.name}", "P", p))

    bd_vrf = {b["name"]: b.get("vrf") for b in snap.get("bds", []) if b.get("vrf")}
    esg_name_vrf = {e["name"]: e.get("vrf") for e in snap.get("esgs", []) if e.get("vrf")}
    for e in snap.get("epgs", []):
        vrf = e.get("vrf") or bd_vrf.get(e["name"]) or esg_name_vrf.get(e["name"]) or primary
        ap_name = e.get("ap") or "network-segments"
        color = RED if "96" in e["name"] else L3
        ap_for(ap_name, vrf).epgs.append((e["name"].replace("_", "/"), color))
        for c in e.get("consumers") or []:
            attachments.append((f"EPG {e['name']}", "C", c))
        for p in e.get("providers") or []:
            attachments.append((f"EPG {e['name']}", "P", p))

    bds_by_vrf: dict[str, list[BD]] = defaultdict(list)
    bds = []
    for b in snap.get("bds", []):
        flags = " + ".join(x for x in ("public", "shared") if x in (b.get("scope") or ""))
        vrf = b.get("vrf") or primary
        obj = BD(b["name"].replace("_", "/"), f"GW  {b.get('ip','')}   {flags}".rstrip(), vrf=vrf)
        bds_by_vrf[vrf].append(obj)
        bds.append(obj)
    ...
    l3_by_vrf = {name: _labels(name) for name in order}
    l3_01 = l3_by_vrf.get(primary) or []
    l3_02 = l3_by_vrf.get(secondary) or [] if secondary else []
    ...
    return Model(
        ...
        l3_vrf02=l3_02[0] if l3_02 else "",
        l3_vrf01=l3_01[0] if l3_01 else "",
        vz_consumers=list(vz_by_vrf.get(primary) or []),
        aps_vrf02=aps_v2,
        aps_vrf01_center=center,
        aps_vrf01_right=right,
        bds=bds,
...
def _object_index(model: Model) -> dict[str, dict]:
    ...
    for vrf, ap in iter_aps(model):
        for name, _c in ap.epgs:
            add(name, "epg", ap.name, vrf)
        for e in ap.esgs:
            add(e.name, "esg", ap.name, vrf)
    return idx
...
def build_graph(model: Model) -> dict:
    ...
    return {
        ...
        "vrfs": {
            "primary": model.vrf_primary,
            "secondary": model.vrf_secondary,
            "all": [ln.name for ln in model_lanes(model)],
        },
        "nodes": sorted(nodes.values(), key=lambda n: (n["ap"], n["kind"], n["label"])),
        "contracts": contracts,
        "links": links,
        "bds": [{"name": b.name, "gw": b.gw, "vrf": b.vrf} for b in model.bds],
        "l3outs": {
            "inside": _l3_labels(model, "01"),
            "outside": _l3_labels(model, "02"),
            **{ln.name: ln.l3outs for ln in model_lanes(model)},
        },
        "notes": model.notes,
        "stats": {
            "nodes": len(nodes),
...
def prompt_cli(job: Job, fmt: str = "png") -> tuple[Job, str]:
    ...
    fmt_in = input(
        f"Output format — pptx / png / svg / both / html / json / all [{fmt}]: "
    ).strip().lower() or fmt
    if fmt_in not in ("pptx", "png", "svg", "both", "html", "json", "all"):
        raise SystemExit("Format must be pptx, png, svg, both, html, json, or all")
...
def fingerprint(model: Model) -> set[tuple]:
    ...
    for vrf, ap in iter_aps(model):
        ...
        for name, _c in ap.epgs:
            items.add((vrf, ap.name, "epg", name))
    for ln in model_lanes(model):
        items.add(("vzAny", ln.name, tuple(sorted(ln.vz_consumers))))
        for bd in ln.bds:
            items.add(("bd", ln.name, bd.name, bd.gw))
        for lab in ln.l3outs:
            items.add(("l3out", ln.name, lab))
    return items
...
def main():
    ...
    parser.add_argument("--format",
                        choices=("pptx", "png", "svg", "both", "html", "json", "all"),
                        default=os.environ.get("DIAGRAM_FORMAT", "pptx"),
                        help="pptx = one widescreen slide (v10 default); "
                             "png/svg static; html interactive viewer; json raw graph; "
                             "both = png+svg; all = pptx+png+svg+html+json")
